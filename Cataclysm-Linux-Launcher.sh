...
function update_launcher() {
    ...
    latest=$(grep /jhagrid77/CDDA-Ubuntu-Launcher/releases/download releases | head -n 1 | tr '"' ' ' | awk '{print $3}' | awk -F "/" '{sub("v", ""); print $6}')
...
function install_cataclysm_compile() {
    if [ "$arch" = 'x86_64' ]; then
        if [ "$ver" = 'Ncurses' ]; then
            ...
            elif [ "$os" = 'RPM-DNF' ]; then
                sudo dnf install astyle."$arch" glib2."$arch" ncurses."$arch"
            #elif [ "$os" = 'RPM-YAST']; then
            elif [ "$os" = 'RPM-YUM' ]; then
...
function install_cataclysm_binary() {
    if [ "$arch" = 'x86_64' ]; then
        ...
        installable=$(awk -F'"' '{print $6}' index.html | grep -E "tar.gz")
        rm ./index.html
        while true; do
            PS3="What version would you like to install?: "
            clear
            echo "Install Menu"
            select install in $installable "Go Back"; do
                case "$install" in
                cataclysm*)
                    if [ "$ver" = 'Ncurses' ]; then
                        echo "Installing needed dependencies"
...
if [ $# = 0 ]; then
    while true; do
        ...
        clear
        echo "Main Menu"
        select question in "Cataclysm" "Backups" "Mods" "Tilesets" "Soundpacks" "Settings" "Update Launcher" "Quit"; do
            case "$question" in
            "Cataclysm")
                while true; do
                    PS3="What would you like to do?: "
                    clear
                    echo "Cataclysm Menu"
                    select question in "Install Cataclysm" "Compile Cataclysm" "Update Cataclysm" "Launch Cataclysm" "Go Back"; do
                        case "$question" in
                        "Install Cataclysm")
                            while true; do
                                ...
                                clear
                                echo "Install Menu"
                                select ver in "Ncurses" "Tiles" "Go Back"; do
                                    case "$ver" in
                                    "Ncurses")
                                        if ! grep -q "^ver=" ./config.sh; then
                                            echo "ver=$ver" | tee -a ./config.sh
                                            source ./config.sh
                                        fi
                                        if [[ $(grep "^ver=$ver" ./config.sh) != ver=* ]]; then
                                            sed -i "s|^ver=.*|ver=$ver|g" ./config.sh
                                            source ./config.sh
                                        fi
                                        install_cataclysm_binary Curses
                                        if ! grep -q "^install=" ./config.sh; then
                                            echo "install=Binary" | tee -a ./config.sh
                                            source ./config.sh
                                        fi
                                        if [[ $(grep "^install=Binary" ./config.sh) != install=* ]]; then
                            ...
                            while true; do
                                ...
                                select ver in "Ncurses" "Tiles" "Go Back"; do
                                    ...
                                    "Ncurses")
                                        ...
                                        if [[ $(grep "^install=Compile" ./config.sh) != install=* ]]; then
